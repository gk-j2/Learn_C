...
//------- NRZ ----------------------------------------
int *nrz(int *bitsIn, int numOfBits) {
	int *bitsOut, i, code;
...
//------- PAM5 ----------------------------------------
int *pam5(int *bitsIn, int numOfBits) {
	int *bitsOut, i, j;

	//write(1, "start mlt3\n", 11);
	bitsOut = (int *)malloc(sizeof(int) * (numOfBits / 2));
...
int main()
{
	...
	printf("Bits after NRZ --> \t\t\t");
	out = nrz(out, len * 8 + len * 2);
	printf("\n");

	printf("Bits after PAM5 --> \t\t");
	out = pam5(out, len * 8 + len * 2);
	printf("\n");
